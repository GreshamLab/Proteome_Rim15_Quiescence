...
```{r, echo = FALSE}
###############################################################################################
pSite<-read_csv("./tables/normed_pSite_ratio_perseus.csv") %>% drop_na(Proteins) %>%
    dplyr::select(-c("id"))
...
lable = logFC_ttest_pS %>% filter( nutrient == "P" & abs(log2FC) > 1 & pvalue < 0.05) %>% filter(!is.na(pvalue))
logFC_ttest_pS %>% filter(!is.na(pvalue)) %>% dplyr::filter(nutrient == "P") %>%
  mutate(trend = case_when(log2FC > 2 & pvalue < 0.05 ~ "Up in WT",
                           log2FC < - 2 & pvalue < 0.05 ~ "Up in RIM15ko",
                           log2FC < 2 | log2FC > -2 | pvalue > 0.05 ~ "non sig")) %>%
  ...
  geom_label_repel(data = lable, aes(label = pSite)) +
  theme(legend.position="none")
ggsave("./plots/pSites_box_P.pdf",width = 7, height = 7,  useDingbats=FALSE)

lable = logFC_ttest_pS %>% filter( nutrient == "C" & abs(log2FC) > 1 & pvalue < 0.05)
logFC_ttest_pS %>% filter(!is.na(pvalue)) %>% dplyr::filter(nutrient == "C") %>%
  mutate(trend = case_when(log2FC > 2 & pvalue < 0.05 ~ "Up in WT",
                           log2FC < - 2 & pvalue < 0.05 ~ "Up in RIM15ko",
                           log2FC < 2 | log2FC > -2 | pvalue > 0.05 ~ "non sig")) %>%
  ggplot(aes(x=time, y =log2FC), color = trend) +
  ...
  theme(legend.position="none")
ggsave("./plots/pSites_box_C.pdf",width = 7, height = 7,  useDingbats=FALSE)
```

```{r}
T00 <- logFC_ttest_pS %>% dplyr::filter(time == "T00") %>% filter(!is.na(pvalue))

lable = T00 %>% filter( nutrient == "C", abs(log2FC) > 1, pvalue < 0.05)
ggplot(data=T00 %>% dplyr::filter(nutrient == "C"), aes(x=log2FC, y =-log10(qvalue))) +
  geom_point(data=dplyr::filter(T00, nutrient == "C", qvalue > 0.05), alpha = 1/6, size = 0.8) +
  geom_point(data=dplyr::filter(T00, nutrient == "C", log2FC < -1, pvalue < 0.05), alpha = 1, size = 1.5,  color = c("#00afbb")) +
  geom_point(data=dplyr::filter(T00, nutrient == "C", log2FC > 1, pvalue < 0.05), alpha = 1, size = 1.5, color = 
c("#e7b800")) +
  theme_classic() +
  #coord_fixed(ratio = 6) +
...
logFC_ttest_pSite %>% write_csv("./tables/DE_pSites_bynutrient.csv")

logFC_ttest_pSite <- read_csv("./tables/DE_pSites_bynutrient.csv")
